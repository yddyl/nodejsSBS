var user = {
    ...
    avgPaceById:"SELECT count(IDnumber) counting, sum(time_to_sec(5km))/60/5/count(IDnumber) AS '0-5km',sum(time_to_sec(10km)-time_to_sec(5km))/60/5/count(IDnumber) AS '5-10km',sum(time_to_sec(15km)-time_to_sec(10km))/60/5/count(IDnumber) AS '10-15km',sum(time_to_sec(20km)-time_to_sec(15km))/60/5/count(IDnumber) AS '15-20km',sum(time_to_sec(21km)-time_to_sec(15km))/60/6.1/count(IDnumber) AS '15-21km',sum(time_to_sec(25km)-time_to_sec(20km))/60/5/count(IDnumber) AS '20-25km',sum(time_to_sec(30km)-time_to_sec(25km))/60/5/count(IDnumber) AS '20-30km',sum(time_to_sec(35km)-time_to_sec(30km))/60/5/count(IDnumber) AS '30-35km',sum(time_to_sec(40km)-time_to_sec(35km))/60/5/count(IDnumber) AS '35-40km',sum(time_to_sec(42km)-time_to_sec(40km))/60/2.195/count(IDnumber) AS '40-42.195km',class from malasong.malasong_chengji where IDnumber=? and (5km<>'00:00:00' or 10km<>'00:00:00'or 15km<>'00:00:00' or 20km<>'00:00:00'or 21km<>'00:00:00'or 25km<>'00:00:00'or 30km<>'00:00:00'or 35km<>'00:00:00'or 40km<>'00:00:00'or 42km<>'00:00:00') group by class",
    avgRecordById:"select count(*) counting, class,sum((rank/(select totalNum from malasong.malasong_bisai where malasong_chengji.matchid=malasong_bisai.matchid)))/count(*) AS avgNewRank,sec_to_time( sum(time_to_sec(finalrecord))/count(*)) avgRecord,42.195/(sum(time_to_sec(finalrecord))/count(*)/60/60) 'avgSpeed(km/h)',sum(time_to_sec(finalrecord))/count(*)/60/42.195 'avgPace(min/km)' from malasong.malasong_chengji where IDnumber=? group by class"
};
