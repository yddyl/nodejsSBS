var user = {
    ...
    chengjiById:'select malasong.malasong_bisai.*,malasong_chengji.*from malasong.malasong_chengji,malasong.malasong_bisai where malasong_bisai.matchid =malasong_chengji.matchid AND malasong.malasong_chengji.IDnumber=?',
    //单个比赛 全部信息，包括选手名字
    ...
    bestRank:'select (rank/(select totalNum from malasong.malasong_bisai where malasong_chengji.matchid=malasong_bisai.matchid)) AS newRank,rank,totalNum,malasong_bisai.*,malasong_chengji.*from malasong.malasong_chengji,malasong.malasong_bisai where malasong_bisai.matchid =malasong_chengji.matchid AND malasong_chengji.IDnumber=? order by newRank limit 10'
};
